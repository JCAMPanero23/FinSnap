...
export const parseTransactions = async (
  ...
  try {
    ...
    if (additionalContext) {
      parts.push({ text: `Additional User Context/Keywords: "${additionalContext}". Use this to better identify merchants, clarify ambiguous text, or aid categorization.` });
    }
...
/**
 * Parses a receipt image and splits the total amount into categorized line items or groups.
 */
export const parseReceiptBreakdown = async (
  imageBase64: string,
  imageMimeType: string,
  totalAmount: number,
  currency: string,
  settings: AppSettings
): Promise<{ item: string; amount: number; category: string }[]> => {
  try {
    const categoryList = settings.categories.map(c => c.name).join(", ");
    
    const SYSTEM_INSTRUCTION = `
      You are a receipt scanning assistant. 
      I will provide an image of a receipt and a Total Transaction Amount: ${totalAmount} ${currency}.
      
      Your goal is to AGGREGATE the receipt items into the user's provided categories.
      Do NOT list every single item unless they belong to different categories.
      
      The user's categories are: [${categoryList}].
      
      Examples of Aggregation:
      - If there are 20 grocery items (Apples, Bread, Milk, etc.), sum them up into ONE entry "Groceries" (or matching Category).
      - If there are 5 toiletry items (Shampoo, Soap), sum them up into ONE entry "Toiletries" (or matching Category).
      - If there are 6 apparel items, sum them up into ONE entry "Clothes".
      
      Constraints:
      - The SUM of all extracted groups MUST equal approximately ${totalAmount}. 
      - Distribute taxes/discounts proportionally or create a separate item if needed.
      - Return a clean list of grouped items.
    `;
    ...
    const response = await ai.models.generateContent({
      ...
      contents: {
        parts: [
          ...
          { text: `Analyze this receipt. The total paid was ${totalAmount}. Aggregate items by category.` }
        ]
      },
      config: {
        systemInstruction: SYSTEM_INSTRUCTION,
        ...
        responseSchema: {
          ...
          items: {
            type: Type.OBJECT,
            properties: {
              item: { type: Type.STRING, description: "Name of the group (e.g. 'Groceries', 'Toiletries')" },
              amount: { type: Type.NUMBER, description: "Total cost of this group" },
              category: { type: Type.STRING, description: "Best matching category from the user list" }
            },
