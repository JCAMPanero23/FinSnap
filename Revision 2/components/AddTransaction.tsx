...
import { Sparkles, ArrowRight, X, Check, Loader2, MessageSquareText, Clipboard, Info, Image as ImageIcon, Banknote, BrainCircuit, Calendar, Pencil, Quote } from 'lucide-react';
...
const AddTransaction: React.FC<AddTransactionProps> = ({ onAdd, onCancel, settings, existingTransactions }) => {
  ...
  // Preview Edit State
  const [editingPreviewItem, setEditingPreviewItem] = useState<Transaction | null>(null);
  ...
  const handleSavePreviewEdit = (updated: Transaction | Transaction[]) => {
    if (!previewData) return;

    if (Array.isArray(updated)) {
      // Split or multiple items returned
      const originalId = updated[0].id;
      setPreviewData(prev => {
        if (!prev) return null;
      ...
      // Single update
      setPreviewData(prev => prev ? prev.map(t => t.id === updated.id ? updated : t) : null);
    }
      ...
      <div className="h-full flex flex-col pb-24 relative">
           ...
           {previewData.map((t) => (
            <div key={t.id} className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 relative group">
              <div className="absolute top-2 right-2 flex gap-1">
                <button 
                  onClick={() => setEditingPreviewItem(t)}
                  className="p-1.5 text-slate-400 hover:text-brand-600 bg-slate-50 rounded-full transition-colors"
                >
                  <Pencil size={14} />
                </button>
                <button 
                  onClick={() => handleRemovePreviewItem(t.id)}
                  className="p-1.5 text-slate-400 hover:text-red-500 bg-slate-50 rounded-full transition-colors"
                >
                  <X size={14} />
                </button>
              </div>
              <div className="flex justify-between items-start mb-2 pr-12">
                <div>
                  <div className="font-bold text-slate-800 text-lg leading-tight">{t.merchant}</div>
                  <div className="text-xs text-slate-500 uppercase tracking-wide font-semibold mt-1">{t.category}</div>
                </div>
                <div className={`font-bold text-lg whitespace-nowrap ${t.type === TransactionType.INCOME ? 'text-green-600' : 'text-slate-800'}`}>
              ...
              <div className="text-xs text-slate-400 flex flex-col gap-1">
                ...
                {t.tags?.includes('Receipt Split') && <span className="text-brand-600 font-bold flex items-center gap-1"><BrainCircuit size={10} /> Split Transaction</span>}
              </div>
            </div>
          ))}
        </div>
        ...
        {editingPreviewItem && (
          <EditTransactionModal 
            transaction={editingPreviewItem}
            categories={settings.categories}
            accounts={settings.accounts}
            onClose={() => setEditingPreviewItem(null)}
            onDelete={(id) => { handleRemovePreviewItem(id); setEditingPreviewItem(null); }}
    ...
    <div className="h-full flex flex-col">
      ...
      {mode === 'ai' && (
        <div className="flex-1 flex flex-col animate-in fade-in slide-in-from-right-4 duration-300">
           ...
           <div className="relative flex-1 mb-6 group flex flex-col gap-3">
              <textarea
              ...
              <div className="space-y-1">
                 <label className="text-xs font-bold text-slate-400 uppercase tracking-wider ml-1">Context / Keywords (Optional)</label>
                 <div className="relative">
                   ...
                   <input 
                     ...
                     placeholder="e.g. 'Trip to Paris', 'Bathroom Renovation', or specific project name"
                     className="w-full pl-9 pr-4 py-3 bg-white rounded-xl border-0 shadow-sm text-sm text-slate-700 focus:ring-2 focus:ring-brand-500 placeholder:text-slate-300"
                   />
                 </div>
                 <p className="text-[10px] text-slate-400 ml-1">Helps AI correctly identify vague merchants or group items.</p>
              </div>
